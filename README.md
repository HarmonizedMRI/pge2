...
referred to here as `pge`. New PulSeg development is moving toward the formal
...
- `pge2.serialize(pge, 'output.pge')`: Export `pge` sequence object to GE binary format
- `pge2.check(pge, sysGE, ...)`: Check compatibility of `pge` sequence object with GE scanner specifications
- `pge2.plot(pge, sysGE, ...)`: Visualize segment/block layout and detailed timing
...
> `pge` adapter may change before a stable PulSeg 2.0 release.
...
3. Convert the PulSeg 2.0-alpha IR to the legacy pge2-compatible `pge` struct:
...
5. Optional: compare output of the WTools simulator with the original Pulseq file:
    ...
    pge2.validate(pge, sysGE, seq, xmlPath, 'row', [], 'plot', true);
    ```
...
Existing `pge2` tools expect the older `pge` representation, including fields such as:
...
The legacy `pge.loop` table does not contain a dedicated ADC frequency offset column.
...
2. Convert to legacy `pge` for existing `pge2` tools.
...
Long-term, direct `pulseg_ir` support is preferred over depending on `pge.loop`.
