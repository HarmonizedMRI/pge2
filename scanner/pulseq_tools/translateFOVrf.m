function psq = translateFOVrf(seq_name, printSHM_output, opuser1, output_file)
% translateFOVrf -- Apply FOV translation to a PulSeg sequence object
% 
% function psq = translateFOVrf(seq_name, printSHM_output)
% 
...
%   printSHM_output   string     Text file containing output of `printSHM`, e.g., `printSHM > Rx.txt`
...
% read z offset
z_offset = pge2.utils.computesliceoffset(printSHM_output)   % cm

% load PulSeg sequence and apply offset
load([seq_name '.mat'])  % psq, params, pislquant
psq = pge2.translateFOVrf(psq, [0 0 z_offset*1e-2]);
